...
ls -la en repositorio clonado -> SOLO DEBEN ESTAR ARCHIVOS .git .gitignore Makefile inc/ src/
make -n 
make
make (no relink)
norminette inc/ inc/libft
...
ls -> verificar que no se ha creado archivo lol.c en de la ejecución del input anterior
...
export TEST=hola
export TEST2=hola
export _ -> BASH no la crea, ni solo _ ni con contenido _=lachota, pero si crea a partir de dos __=lachota ... =/
unset TEST
unset TEST2
unset
unset asdad
...
unset unset unset unset /USER PWD -> $? = 1 (BASH) nosotros nos quedamos con	 $? = 0 por última ejecución de UNSET
cd -
cd ../
cd
export -> check PWD / OLDPWD
unset HOME | cd
unset PWD OLDPWD | cd ..
